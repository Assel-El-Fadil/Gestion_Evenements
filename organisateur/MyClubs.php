...
<body>
    ...
    <div class="app-container">
        ...
        <div class="main-content">
                ...
                <div class="header-content">
                    <div class="header-top">
                        <div class="header-title">
                            <h1>Mes Clubs</h1>
                            <p>Gérez vos adhésions aux clubs et restez connecté</p>
                        </div>
                    ...
                    <div class="stats-grid">
                        <div class="stat-card">
                            <div class="stat-header">
                                <p class="stat-label">Total des Clubs</p>
                        ...
                        <div class="stat-card">
                            <div class="stat-header">
                                <p class="stat-label">En Gestion</p>
                                <div class="stat-icon purple">⭐</div>
                            </div>
                            <p class="stat-value"><?php echo $managing_clubs; ?></p>
                            <p class="stat-meta">Postes de direction</p>
                        </div>

                        <div class="stat-card">
                            <div class="stat-header">
                                <p class="stat-label">Membre de</p>
                                <div class="stat-icon green">👥</div>
                            </div>
                            <p class="stat-value"><?php echo $member_of_clubs; ?></p>
                            <p class="stat-meta">Adhésions actives</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="content-area">
                <div class="tabs-list">
                    <button class="tab-trigger active" onclick="switchTab('all')">Tous (<?php echo isset($user_clubs_count) ? $user_clubs_count : (isset($clubs) ? count($clubs) : 0); ?>)</button>
                    <button class="tab-trigger" onclick="switchTab('managing')">En Gestion (<?php echo $managing_clubs; ?>)</button>
                ...
                <div id="tab-all" class="tab-content active">
                    <div class="clubs-grid">
                        <?php
                        if (empty($clubs)) {
                            echo '<div style="text-align:center; color:#9ca3af; grid-column:1 / -1; padding:24px;">Aucun club trouvé</div>';
                        } else {
                ...
                <div id="tab-managing" class="tab-content">
                    <div class="clubs-grid">
                        ...
                        if (empty($managing_clubs_list)) {
                            echo '<div style="text-align:center; color:#9ca3af; grid-column:1 / -1; padding:24px;">Aucun rôle de gestion trouvé</div>';
                        } else {
                ...
                <div id="tab-member" class="tab-content">
                    <div class="clubs-grid">
                        ...
                        if (empty($member_clubs_list)) {
                            echo '<div style="text-align:center; color:#9ca3af; grid-column:1 / -1; padding:24px;">Aucune adhésion à un club trouvée</div>';
                        } else {
    ...
    <script>
        ...
        async function showClubDetails(id) {
            try {
                toggleModal(true, '<div class="muted">Chargement...</div>');
                ...
                const body = `
                    ...
                    <div class="muted" style="margin:12px 0 6px;">Événements</div>
                    <div class="events-list">
                        ${Array.isArray(events) && events.length > 0 ? events.map(ev => `
                            <div class=\"event-row\">
                                <div>
                                    <div style=\"font-weight:600\">${ev.nom}</div>
                                    <div class=\"muted\">${ev.date_evenement} • ${ev.lieu || 'À déterminer'}</div>
                                </div>
                                <div class=\"muted\">${ev.participants_inscrits || 0} participants</div>
                            </div>
                        `).join('') : '<div class="muted">Aucun événement pour ce club</div>'}
                    </div>
                `;
                setModalContent(club.nom || 'Détails du Club', body);
            } catch (e) {
                setModalContent('Erreur', '<div class="muted">Échec du chargement des détails du club: ' + e.message + '</div>');
            }
        }
    ...
    <div id="clubModal" class="modal-backdrop" onclick="if(event.target===this) toggleModal(false)">
        <div class="modal">
            <div class="modal-header">
                <div id="clubModalTitle" class="modal-title">Détails du Club</div>
